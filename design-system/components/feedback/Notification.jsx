...
export function Notification({ tone = "info", title, children, onClose, toast = false, style }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ width: 32, height: 32, flex: "none", borderRadius: "var(--radius-sm)", background: t.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={t.color.startsWith("var") ? undefined : t.color} style={{ stroke: `${t.color}` }} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{icons[tone] || icons.info}</svg>
      </span>
      ...
      {onClose && (
        <button onClick={onClose} aria-label="Tutup" style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: "var(--pk-text-secondary)", lineHeight: 0 }}>
