...
/**
 * Modal terpusat — raised surface, radius 20, elevation-3, overlay gelap.
...
export function Modal({ open = false, title, children, footer, onClose, width = 480, style }: ModalProps) {
  if (!open) return null;
  return (
    <div
      onClick={(e) => {
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 24px 0" }}>
          ...
          {onClose && (
            <button
              onClick={onClose}
              aria-label="Tutup"
              style={{ background: "none", border: "none", cursor: "pointer", padding: 6, color: "var(--pk-text-secondary)", lineHeight: 0 }}
            >
        ...
        {footer && <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, padding: "0 24px 20px" }}>{footer}</div>}
      </div>
    </div>
  );
}
